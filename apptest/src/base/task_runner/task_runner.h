...
public:
  ...
  template<typename Cb>
  void post_task(cancellation_token tok, Cb&& callback, task::delay_type delay);

  void post_task(task::callback_type callback, task::run_time_type run_time);
  template<typename Cb>
  void post_task(cancellation_token tok, Cb&& callback, task::run_time_type run_time);
...
template<typename Cb>
void task_runner::post_task(cancellation_token tok, Cb&& callback, task::delay_type delay) {
  post_task([callback = std::forward<Cb>(callback), tok = std::move(tok)] {
    if (!tok.is_canceled()) {
      callback();
    }
...
template<typename Cb>
void task_runner::post_task(cancellation_token tok, Cb&& callback, task::run_time_type run_time) {
  post_task([callback = std::forward<Cb>(callback), tok = std::move(tok)] {
    if (!tok.is_canceled()) {
      callback();
    }
